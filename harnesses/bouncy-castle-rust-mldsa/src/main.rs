...
fn handle_sign(req: &Request) -> Response {
    ...
    match param_set {
        44 => {
            ...
            let signature = match MLDSA44::sign_mu_deterministic(&sk, None, &mu, rnd) {
                Ok(s) => s,
        ...
        65 => {
            ...
            let mu = compute_mu(sk.tr(), &message);
            let signature = match MLDSA65::sign_mu_deterministic(&sk, None, &mu, rnd) {
                Ok(s) => s,
                Err(e) => return err(format!("sign failed: {e:?}")),
            };
            outputs.insert("signature".into(), hex::encode(&signature));
        ...
        87 => {
            ...
            let mu = compute_mu(sk.tr(), &message);
            let signature = match MLDSA87::sign_mu_deterministic(&sk, None, &mu, rnd) {
                Ok(s) => s,
                Err(e) => return err(format!("sign failed: {e:?}")),
            };
            outputs.insert("signature".into(), hex::encode(&signature));
...
fn handle_verify(req: &Request) -> Response {
    ...
    let valid: bool = match param_set {
        44 => {
            ...
            MLDSA44::verify_mu_internal(&pk, &pk.A_hat(), &mu, sig)
        },
        65 => {
            ...
            let mu = compute_mu(&pk.compute_tr(), &message);
            MLDSA65::verify_mu_internal(&pk, &pk.A_hat(), &mu, sig)
        },
        87 => {
            ...
            let mu = compute_mu(&pk.compute_tr(), &message);
            MLDSA87::verify_mu_internal(&pk, &pk.A_hat(), &mu, sig)
        },
        _ => return err(format!("unsupported param_set: {param_set}")),
    };
